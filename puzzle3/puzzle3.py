...
def goal(gameState, max_moves=10):
    # length of the valid actions is greater than 10 and the act man is alive with score not equal to zero it is a victory.
    if len(gameState.validActions) >= max_moves and (gameState.actManPos is not None):
        return True
    # if actman position is none then the game ends it is not a victory.
    elif gameState.actManPos is None:
        return False
...
def getValidActions(state: GameState):
    ...
    if (antManPos != None):
        ...
        for action in [[-1, 0, Direction.North], [-1, 1, Direction.North_East],[0, 1, Direction.East],[1, 1, Direction.South_East],[1, 0, Direction.South],[1, -1, Direction.South_West],[0, -1, Direction.West],[-1, -1, Direction.North_West]]:
...
def TransitionFunction(s, p):
    # for all the actions in p perform the specified operations and return the state.
    for action in p:
        if (action[3] == True):
...
def Act_Man_2_ID_DFS(s0):
    depth = 0
    res = True
    while (res):
        state = Act_Man_Bounded_DFS(s0, depth)
        if isinstance(state, GameState):  
            return state
        res = state
        depth = depth + 1

def Act_Man_Bounded_DFS (s,depth):
    frontier = deque([])
    limit_hit = False
...
def main():
    ...
    # call the DFS
    result = Act_Man_2_ID_DFS(gameState)

    # Write output file
    writeOutputFile(sys.argv[2], result if result else prev_ans)
